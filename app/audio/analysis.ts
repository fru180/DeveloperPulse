...
export const SPECTRUM_RANGES: ReadonlyArray<readonly [number, number]> = Array.from(
  { length: SPECTRUM_BAND_COUNT },
  (_, index) => {
    const low = SPECTRUM_MIN_HZ * (SPECTRUM_MAX_HZ / SPECTRUM_MIN_HZ) ** (index / SPECTRUM_BAND_COUNT);
    const high = SPECTRUM_MIN_HZ * (SPECTRUM_MAX_HZ / SPECTRUM_MIN_HZ) ** ((index + 1) / SPECTRUM_BAND_COUNT);
...
export function aggregateSpectrumData(
  ...
  return SPECTRUM_RANGES.map(([low, high]) => {
    ...
    const end = Math.min(frequencyDb.length, Math.max(start + 1, Math.ceil(high / hzPerBin)));
    let power = 0;
    let count = 0;
    for (let index = start; index < end; index += 1) {
      const db = Number.isFinite(frequencyDb[index]) ? frequencyDb[index] : -100;
      power += 10 ** (db / 10);
      count += 1;
    }
    return Math.max(-100, 10 * Math.log10(Math.max(power / Math.max(1, count), 1e-10)));
  });
}
...
export function aggregateLiveBands(spectrumDb: SpectrumDb): number[] {
  return Array.from({ length: COLUMN_COUNT }, (_, columnIndex) => {
    const start = Math.round(columnIndex * SPECTRUM_BAND_COUNT / COLUMN_COUNT);
    const end = Math.round((columnIndex + 1) * SPECTRUM_BAND_COUNT / COLUMN_COUNT);
    let power = 0;
    for (let index = start; index < end; index += 1) {
      const db = Number.isFinite(spectrumDb[index]) ? spectrumDb[index] : -100;
      power += 10 ** (db / 10);
    }
    return Math.max(-100, 10 * Math.log10(Math.max(power / Math.max(1, end - start), 1e-10)));
  });
}
...
export function smoothBands(previous: BandDb | null, next: BandDb): BandDb {
  ...
  return next.map((value, index) =>
    EMA_ALPHA * value + (1 - EMA_ALPHA) * previous[index],
  ) as BandDb;
}
...
export function bandsToColumn(bands: BandDb, sensitivityDb: number): IntensityColumn {
  return bands.map((value) => dbToIntensity(value, sensitivityDb)) as IntensityColumn;
}
...
export function pushColumn(grid: IntensityColumn[], column: IntensityColumn): IntensityColumn[] {
...
export function cellTargets(
  ...
  profiles.forEach((profile, index) => {
    ...
    targets[index] = Math.max(0, Math.min(1, (adjustedDb - LIVE_MIN_DB) / (LIVE_MAX_DB - LIVE_MIN_DB)));
  });
  return targets;
}
...
export function cellPeakSignals(
  ...
  profiles.forEach((profile, index) => {
    ...
    const adjustedDb = db + sensitivityDb;
    const energy = Math.max(0, Math.min(1, (adjustedDb - LIVE_MIN_DB) / (LIVE_MAX_DB - LIVE_MIN_DB)));
